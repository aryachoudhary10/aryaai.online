...
const COMPANIES = ["Google","Apple","Microsoft","Amazon","Meta","Facebook","Netflix","HSBC","Zara","Tesla","Nvidia","Infosys","TCS","Wipro","Adobe","Spotify","Uber","Airbnb","OpenAI","Anthropic","Google","Samsung","Sony","Intel","IBM","Oracle","Salesforce","Flipkart","Swiggy","Zomato","Stripe"];
...
function heuristic(text, today){
  ...
  if(/\b(remind me|i should|i need to|i promised|don'?t forget|have to|call)\b/i.test(text)){
    const txt=text.replace(/^\s*remind me (on [^,]+,?\s*)?to\s*/i,"").replace(/\.$/,"");
    out.reminders.push({text:txt,dateIso:d&&d.date?iso(d.date):null,recurring:d&&d.recurring?d.recurring:null,label:d?d.label:"soon",isBirthday:false,who});
  }
...
function normalize(o){
  ...
  return {
    ...
    reminders: arr(o.reminders).filter(r=>r&&r.text).map(r=>({text:String(r.text),dateIso:r.dateIso||null,recurring:r.recurring||null,label:r.label||"soon",isBirthday:!!r.isBirthday,who:r.who||null})),
