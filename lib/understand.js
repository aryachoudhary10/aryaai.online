...
function heuristic(text, today){
  ...
  if(/\b(remind me|i should|i need to|i promised|don'?t forget|have to|call)\b/i.test(text)){
    const txt=text.replace(/^\s*remind me (on [^,]+,?\s*)?(at [^,]+,?\s*)?to\s*/i,"").replace(/\.$/,"");
    const tm=parseTime(text);
    const rep=parseRepeat(text);
    const dIso = d&&d.date ? iso(d.date) : ((tm||rep) ? iso(today) : null); // time/repeat with no date means today
    if(rep&&tm&&(rep.type==="daily"||rep.type==="weekly"))rep.time=tm;
    out.reminders.push({text:txt,dateIso:dIso,time:tm,repeat:rep,recurring:d&&d.recurring?d.recurring:null,label:d?d.label:"soon",isBirthday:false,who});
  }
...
function normalize(o){
  ...
  return {
    ...
    reminders: arr(o.reminders).filter(r=>r&&r.text).map(r=>({text:String(r.text),dateIso:r.dateIso||null,time:r.time||null,repeat:r.repeat&&typeof r.repeat==="object"?r.repeat:null,recurring:r.recurring||null,label:r.label||"soon",isBirthday:!!r.isBirthday,who:r.who||null})),
