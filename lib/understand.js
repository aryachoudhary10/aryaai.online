...
const iso = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`; // LOCAL date (avoid UTC off-by-one)
const localStamp = (d) => { const days=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"]; const p=(n)=>String(n).padStart(2,"0"); return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())} (${days[d.getDay()]})`; };
...
// Actually call Gemini to verify the saved key + model really work.
export async function testKey() {
  if (!hasKey()) throw new Error("No API key saved.");
  await gemini("Reply with the single word: OK.", "ping", false);
...
function parseDate(text, today){
  ...
  // month-name dates FIRST (so "8th april" isn't misread as "the 8th"): "8 april","8th april","april 8","april 8th"
  const MN="january|february|march|april|may|june|july|august|september|october|november|december";
  let dm=lo.match(new RegExp("\\b(\\d{1,2})(?:st|nd|rd|th)?\\s+("+MN+")\\b"));
  if(!dm){const md2=lo.match(new RegExp("\\b("+MN+")\\s+(\\d{1,2})(?:st|nd|rd|th)?\\b"));if(md2)dm=[md2[0],md2[2],md2[1]];}
  if(dm){const day=+dm[1];const mi=MONTHS.findIndex(x=>x.toLowerCase()===dm[2]);if(mi>=0)return{date:new Date(today.getFullYear(),mi,day),label:`${day} ${MONTHS[mi]}`};}
  // "on the 8th" → day of month (only when no month name was given)
  m=lo.match(/on (?:the )?(\d{1,2})(?:st|nd|rd|th)?\b/);
...
function heuristic(text, today){
  ...
  if(/\b(remind me|i should|i need to|i promised|don'?t forget|have to|call)\b/i.test(text)){
    ...
    let dIso = offM ? null : (d&&d.date ? iso(d.date) : ((tm||rep) ? iso(today) : null)); // time/repeat with no date means today
    if(!offM && tm && !(d&&d.date) && !rep){ const [hh,mm]=tm.split(":").map(Number); const cand=new Date(today); cand.setHours(hh,mm,0,0); if(cand.getTime()<Date.now()) cand.setDate(cand.getDate()+1); dIso=iso(cand); } // roll past clock-time to tomorrow
