...
export async function cancelScheduled(id) {
  const res = await fetch("/api/cancel", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ deviceId: deviceId(), id }) });
  return res.json();
}
...
export async function scheduleReminders(reminders, ctx) {
  ...
  for (const r of reminders) {
    ...
    // 0) Relative offset — "remind me in 10 minutes" → fire once at now + offset
    if (r.offsetMin && r.offsetMin > 0) {
      const c = await copyFor(r, ctx, "due");
      items.push({ id: rid(), fireAt: new Date(now + r.offsetMin * 60000).toISOString(), title: c.title, body: c.body, url: "/" });
      continue;
    }
